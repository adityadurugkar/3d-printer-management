  ...
  return (
    <div className="space-y-6 pb-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Notifications</h1>
              <p className="text-sm text-gray-600 dark:text-white/40">System alerts and updates</p>
            </div>
          </div>
          {localNotifs.length > 0 && (
            <Button
              ...
              size="sm"
              onClick={markAllRead}
              className="text-xs text-gray-600 dark:text-white/40 hover:text-gray-900 dark:hover:text-white h-8"
            >
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        {localNotifs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-500 dark:text-white/30">
          ...
          <div className="divide-y divide-gray-200 dark:divide-white/[0.04]">
            {localNotifs.map((notif, i) => {
              const cfg = typeConfig[notif.type] || { icon: Bell, color: 'text-gray-500 dark:text-white/50', bg: 'bg-gray-100 dark:bg-white/[0.04]', label: 'Notification' }
              const Ic = cfg.icon
              return (
                <motion.div
                  key={notif._id || i}
                  ...
                  className="flex items-start gap-4 px-5 py-4 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-gray-600 dark:text-white/50 uppercase tracking-wider">{cfg.label}</span>
                      {!notif.read && <span className="w-1.5 h-1.5 rounded-full bg-primary" />}
                    </div>
                    <p className="text-sm font-medium text-gray-800 dark:text-white/80 mt-0.5">{notif.title || notif.message}</p>
                    <p className="text-xs text-gray-600 dark:text-white/40 mt-0.5">{notif.description || ''}</p>
                  </div>
                  <span className="text-[11px] text-gray-500 dark:text-white/30 flex-shrink-0">
