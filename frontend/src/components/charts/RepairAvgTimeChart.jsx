...
export default function RepairAvgTimeChart({ data = [] }) {
  return (
    <div className="bg-card border border-border/40 rounded-2xl p-5 sm:p-6 shadow-sm">
      <h3 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">Performance</h3>
      <h2 className="text-base font-bold text-foreground mb-5">Avg Repair Time per Technician</h2>
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data} barSize={36} barCategoryGap="20%">
            ...
            <YAxis tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} width={40} unit="h" />
            <Tooltip
